...
export abstract class Renderer extends Module {
    ...
    private activeDot: HTMLSpanElement | undefined =
    ...
    private csvLog: string | undefined =
    ...
    constructor(moduleName: string) {
        ...
        this.postprocessingBufferA = new THREE.WebGLRenderTarget(
            window.innerWidth,
            window.innerHeight,
            { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter }
        );
        this.postprocessingBufferB = new THREE.WebGLRenderTarget(
            window.innerWidth,
            window.innerHeight,
            { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter }
        );
    ...
    protected async onUpdateImageDataInput(): Promise<void> {
        ...
        this.clearRendering();
        if (
            this.imageDataInputs?.[0].data !== undefined &&
            this.imageDataInputs?.[0].data.data != null &&
            this.canvas &&
            this.scene
        ) {
        ...
        if (
            ...
            if (
                ...
                if (
                    ...
                    this.imageDataInputs[0].shader.kernels.length > 0
                ) {
            ...
            if (this.imageDataInputs?.[0].data) {
                ...
                if (this.elapsedTime > 1000) {
                    const fps = Math.round(
                        (this.frames * 1000) / this.elapsedTime
                    );
                    if (this.htmlFps) {
                ...
                if (
                    ...
                    this.totalTimeElapsed >= this.timeUntilLogging &&
                    !this.cvsProvided
                ) {
                    if (
                        ...
                        const averageFps = Math.round(
                            (this.totalFrames * 1000) / this.totalTimeElapsed
                        );
                        this.csvLog =
                            this.csvLog +
