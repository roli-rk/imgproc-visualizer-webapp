...
export abstract class Module extends HTMLElement {
    ...
    private root: HTMLElement | undefined = document.querySelector(
        ':root'
    ) as HTMLElement;

    private htmlInput: HTMLElement | undefined =
        document.createElement('section');
    private htmlOutput: HTMLElement | undefined =
        document.createElement('section');
        ...
        if (this.htmlInput && this.inputElements) {
            ...
            if (this.imageDataInputs) {
                for (let i = 0; i < inputs[0]; i++) {
                    ...
                    this.inputElements?.push(
                        new ImageDataInput(
                            () => this.onUpdateInputImageDataCallback(),
                            newImageData,
                            this.htmlInput
                        )
                    );
            ...
            if (this.sceneDataInputs) {
                for (let i = 0; i < inputs[1]; i++) {
                    ...
                    this.inputElements.push(
                        new SceneInput(
                            () => this.onUpdateInputSceneCallback(),
                            newSceneObject,
                            this.htmlInput
                        )
                    );
            ...
            if (this.baseObjectInputs) {
                for (let i = 0; i < inputs[2]; i++) {
                    ...
                    this.inputElements.push(
                        new BaseObjectInput(
                            () => this.onUpdateInputBaseObjectCallback(),
                            newBaseObject,
                            this.htmlInput
                        )
                    );
            ...
            if (this.imageDataOutputs && this.outputImageDataElements) {
                for (let i = 0; i < outputs[0]; i++) {
                    ...
                    this.outputImageDataElements.push(
                        new ImageDataOutput(newImageData, this.htmlOutput)
                    );
                }
            }
            ...
            if (this.sceneDataOutputs && this.outputSceneElements) {
                for (let i = 0; i < outputs[1]; i++) {
                    ...
                    this.outputSceneElements.push(
                        new SceneOutput(newSceneObject, this.htmlOutput)
                    );
                }
            }
            ...
            if (this.baseObjectsOutputs && this.outputBaseObjectElements) {
                for (let i = 0; i < outputs[2]; i++) {
                    ...
                    this.outputBaseObjectElements.push(
                        new BaseObjectOutput(newBaseObject, this.htmlOutput)
                    );
                }
            }
        }
            ...
            (event) => {
                if (this.clicked && !event.ctrlKey) {
                    if (
                        ...
                        this.outputBaseObjectElements.forEach(
                            (output: IOutput) => {
                                output.updateStartLine(
                                    event.movementX,
                                    event.movementY
    ...
    private releaseResources(): void {
        ...
        this.imageDataOutputs?.forEach((outputElement: ImageDataObject) => {
            if (outputElement.data) outputElement.data.data = undefined;
        });
        this.sceneDataOutputs?.forEach((outputElement: SceneObject) => {
            if (outputElement.data) outputElement.data = undefined;
        });
