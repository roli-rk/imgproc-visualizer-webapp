...
export function addEventListeners(
    ...
    rootElement.querySelectorAll('*').forEach((element) => {
        events.forEach((eventType) => {
            const actionAttribute = element.getAttribute(`(${eventType})`);
            if (actionAttribute) {
                console.log(
                    `Adding event listener for ${eventType} on element:`,
                    element
                );
...
function handleEvent(event: Event, context: HTMLElement) {
    ...
    while (target && target !== context) {
        ...
        if (action) {
            ...
            if (match) {
                ...
                const params = paramNames.map((paramName) => {
                    ...
                    // Check whether the parameter is a string (in quotation marks)
                    if (/^['"].*['"]$/.test(paramName)) {
                        return paramName.slice(1, -1); // Removes the quotation marks
                    }
                    ...
                    // Check whether the parameter exists in the context
                ...
                // Check whether one of the parameters is undefined and whether parameters were expected
                ...
                // If no parameters are passed, we call the method directly
        ...
        // If the target does not have an event handler, go to the parent element
