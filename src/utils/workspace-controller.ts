        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (x1 && x2 && y1 && y2) {
                lines[i].setAttribute(
                    'x1',
                    ((parseFloat(x1) - lastLeft) / lastWidth) *
                        modulesHtmlClientRect.width +
                        modulesHtmlClientRect.x +
                        'px'
                );
                lines[i].setAttribute(
                    'x2',
                    ((parseFloat(x2) - lastLeft) / lastWidth) *
                        modulesHtmlClientRect.width +
                        modulesHtmlClientRect.x +
                        'px'
                );
                lines[i].setAttribute(
                    'y1',
                    ((parseFloat(y1) - lastTop) / lastHeight) *
                        modulesHtmlClientRect.height +
                        modulesHtmlClientRect.y +
                        'px'
                );
                lines[i].setAttribute(
                    'y2',
                    ((parseFloat(y2) - lastTop) / lastHeight) *
                        modulesHtmlClientRect.height +
                        modulesHtmlClientRect.y +
                        'px'
                );
    ...
    if (svgConnectLines) {
        ...
        if (modulesHtml) {
            ...
            if (scale != 1) {
                ...
                let modulesHtmlClientRect =
                    modulesHtml?.getBoundingClientRect();
                lastWidth = modulesHtmlClientRect.width;
                lastHeight = modulesHtmlClientRect.height;
                lastLeft = modulesHtmlClientRect.x;
                lastTop = modulesHtmlClientRect.y;
...
const view = (() => {
    ...
    const API = {
        ...
        scaleAt(at: Position, amount: number) {
            // at in screen coords
            if (dirty) {
                this.update();
            }
